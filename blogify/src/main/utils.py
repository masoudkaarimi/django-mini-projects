...
def to_jalali(input_date, output_format='%Y-%m-%d'):
    ...
    Args:
        input_date (datetime.date, datetime.datetime, str): The input date.
        output_format (str): The output format. Default is 'YYYY-MM-DD'.

    Returns:
        str: The Jalali date as a string.
    """
...
def toast(request, message, level="info"):
    ...
    Args:
        ...
        message (str, dict): The message to display.
        level (str): The message level. Default is 'info'. Can be 'debug', 'info', 'success', 'warning', or 'error'.

    Raises:
        ValueError: If the message level is invalid.
...
def generate_upload_path(instance, filename, prefix='uploads', filed_name=None):
    """
    ...
    Args:
        ...
        prefix: The path prefix. Default is 'uploads'.
        filed_name: The field name. Default is None.

    Returns:
        str: The unique upload path.
    """
    timestamp = timezone.now().strftime('%Y%m%d_%H%M%S')
    extension = filename.rsplit('.', 1)[-1]

    if filed_name:
        return f'{prefix}/{filed_name}/{timestamp}.{extension}'
